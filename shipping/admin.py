...
class CarrierAdmin(admin.ModelAdmin):
    ...
    def test_fedex_token_view(self, request, pk):
        from .fedex_client import FedExClient, FedExError
        carrier = get_object_or_404(Carrier, pk=pk)
        try:
            client = FedExClient(carrier=carrier)
            token = client.get_token()
            return JsonResponse({
                'ok': True,
                'message': f'Токен отримано ({len(token)} симв.)',
    ...
    def test_fedex_rates_view(self, request, pk):
        from .fedex_client import FedExClient, FedExError
        carrier = get_object_or_404(Carrier, pk=pk)
        origin = (carrier.sender_country or 'DE').upper()
        _test_dest = {
...
class ShipmentAdmin(AuditableMixin, admin.ModelAdmin):
    ...
    def _handle_edit_post(self, request, shipment, carriers):
        ...
        if pkg_weights:
            ...
            # Оновити shipment.weight_kg як сума всех коробок
            total_weight = sum(float(p.weight_kg or 1) * (p.quantity or 1) for p in shipment.packages.all())
            if total_weight > 0:
                shipment.weight_kg = total_weight
                shipment.save(update_fields=["weight_kg"])

            # Оновити ваги в customs_articles тільки якщо вони ще не задані ручно
            if shipment.customs_articles:
                ...
                if pkg_items_distribution_edit and packages:
                    # Якщо є розподіл товарів — використати його для авто-ваг
                    for idx, article in enumerate(articles):
                        # НЕ перезаписувати ручно задані ваги (weight_auto=False)
                        if article.get("weight_auto") != False:
                            for pkg in packages:
                                if pkg.items_distribution:
                                    for dist_item in pkg.items_distribution.get("items", []):
                                        if dist_item.get("index") == idx:
                    ...
                    # Якщо розподіл не заповнений — розраховувати для авто-ваг
                    ...
                    for idx in range(len(articles)):
                        # НЕ перезаписувати ручно задані ваги (weight_auto=False)
                        if articles[idx].get("weight_auto") != False:
    ...
    def shipment_detail_view(self, request, shipment_id):
        ...
        if is_jumingo and shipment.jumingo_order_number:
            _lbl_filename = (
                (_label_display_url.split("/")[-1] if _label_display_url else "")
                or f"label-{shipment.jumingo_order_number}.pdf"
            )
    ...
    def jumingo_document_view(self, request, shipment_id, doc_type, filename):
        """Повертає PDF документ Jumingo через base64-декодування з /v1/orders/{id}/documents.

        Відповідь API: {
          "labels":        {"s_xxx": {"name": "L-xxx.pdf", "type": "label", "file": "<base64>"}},
          "invoices":      [{"name": "R-xxx.pdf", "type": "invoice",            "file": "<base64>"}],
          "confirmations": [{"name": "A-xxx.pdf", "type": "orderconfirmation",  "file": "<base64>"}],
        }
        """
        import base64
        ...
        try:
            ...
            return HttpResponse(f"Jumingo API error: {e}", status=502,
                                content_type="text/plain; charset=utf-8")
        ...
        if doc_type == "label":
            ...
            for v in items:
                if isinstance(v, dict) and v.get("file"):
                    pdf_b64 = v["file"]
                    out_filename = v.get("name") or filename
                    break
        elif doc_type == "invoice":
            for item in (docs_data.get("invoices") or []):
                if isinstance(item, dict) and item.get("file"):
                    pdf_b64 = item["file"]
                    out_filename = item.get("name") or filename
                    break
        elif doc_type in ("orderconfirmation", "confirmation"):
            for item in (docs_data.get("confirmations") or []):
                if isinstance(item, dict) and item.get("file"):
                    pdf_b64 = item["file"]
                    out_filename = item.get("name") or filename
        ...
        if not pdf_b64:
            return HttpResponse(
                f"Документ '{doc_type}' не знайдено у замовленні {order_num}.\n"
                f"Завантажте вручну: https://app.jumingo.com/orders/{order_num}",
        ...
        try:
            ...
            return HttpResponse(f"PDF decode error: {e}", status=502,
                                content_type="text/plain; charset=utf-8")
    ...
    def book_view(self, request, shipment_id):
        ...
        if order_number:
            ...
            # Перевірити наявність мітки (labels — dict в API Jumingo, file=base64)
            docs = service.get_order_documents(order_number)
            labels = docs.get("labels") or {}
            label_items = labels.values() if isinstance(labels, dict) else labels
            has_label = any(isinstance(v, dict) and v.get("file") for v in label_items)
            if has_label:
                messages.success(request, f"✅ Замовлення оформлено! #{order_number}. Етикетка готова.")
            else:
    ...
    def tracking_badge(self, obj):
        ...
        _PRN = (
            '<br><span style="display:inline-block;margin-top:3px;padding:1px 7px;'
