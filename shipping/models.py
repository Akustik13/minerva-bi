...
class UPSConfig(models.Model):
    """Налаштування UPS API. Singleton pk=1."""

    client_id     = models.CharField('Client ID', max_length=200, blank=True,
                      help_text='UPS Developer Portal → Apps → Credentials')
    ...
    account_number = models.CharField('Account Number', max_length=50, blank=True,
                       help_text='UPS Account Number для виставлення рахунків')

    use_sandbox = models.BooleanField('Тестовий режим (Sandbox)', default=True,
                    help_text='Sandbox: wwwcie.ups.com / Production: onlinetools.ups.com')
    is_enabled  = models.BooleanField('Увімкнути UPS', default=False)

    # Кешований OAuth токен — не редагувати вручну
    _cached_token     = models.CharField(max_length=2000, blank=True, db_column='cached_token')
    _token_expires_at = models.DateTimeField(null=True, blank=True, db_column='token_expires_at')

    # Адреса відправника
    shipper_name    = models.CharField('Ім\'я відправника', max_length=200, blank=True)
    shipper_address = models.CharField('Адреса', max_length=300, blank=True)
    shipper_city    = models.CharField('Місто', max_length=100, blank=True)
    shipper_state   = models.CharField('Штат/регіон', max_length=10, blank=True,
                        help_text='США: 2 букви (NY). Інші: можна порожньо')
    shipper_postal  = models.CharField('Поштовий індекс', max_length=20, blank=True)
    shipper_country = models.CharField('Код країни', max_length=2, blank=True, default='DE',
                        help_text='2 букви: DE, UA, US, PL...')
    shipper_phone   = models.CharField('Телефон', max_length=30, blank=True)

    LABEL_FORMAT_CHOICES = [
        ('PDF', 'PDF — для звичайного принтера'),
        ('PNG', 'PNG — зображення'),
        ('ZPL', 'ZPL — термо-принтер Zebra'),
        ('GIF', 'GIF — зображення'),
    ]
    label_format = models.CharField('Формат мітки', max_length=5,
                     choices=LABEL_FORMAT_CHOICES, default='PDF')

    paperless_trade = models.BooleanField('Paperless Trade', default=False,
                        help_text='Електронна митна декларація. Потребує активації в UPS акаунті.')
    eori_number = models.CharField('EORI номер', max_length=50, blank=True,
                    help_text='Напр.: DE123456789')
    vat_number  = models.CharField('VAT номер', max_length=50, blank=True)

    last_sync_at     = models.DateTimeField('Остання синхронізація', null=True, blank=True)
    ...
    @property
    def base_url(self):
        return 'https://wwwcie.ups.com' if self.use_sandbox else 'https://onlinetools.ups.com'

    def is_configured(self):
        return bool(self.client_id and self.client_secret and self.account_number)
