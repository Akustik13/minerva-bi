...
def get_invoice_number_info(config=None) -> dict:
    """
    Compare last invoice number from /invoices/ (Invoice model + DOCX files) vs DigiKey supplierInvoiceNumber.
    Returns dict:
      {
        sys_last:   int,   # max invoice number in our Invoice model + media/invoices/ files
    ...
    # Also check file system (Invoice_*.docx in media/invoices/)
    file_max = 0
    output_dir = Path(settings.MEDIA_ROOT) / "invoices"
    if output_dir.exists():
        for f in output_dir.glob("Invoice_*.docx"):
