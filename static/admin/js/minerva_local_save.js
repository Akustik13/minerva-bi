...
(function () {
  ...
  /**
   * Записує Blob у вказаний файл всередині handle-директорії.
   * Повертає Promise<void>.
   */
  function writeBlob(handle, filename, blob) {
  ...
  /**
   * Зручна функція: завантажує файл за URL і зберігає в обрану папку.
   * Повертає Promise<{ok, folderName}> або {ok: false, reason: '...'}.
   */
  function saveUrlToFolder(url, filename) {
    return getHandle().then(function (handle) {
      ...
      return fetch(url).then(function (r) {
        ...
        return writeBlob(handle, filename, blob).then(function () {
