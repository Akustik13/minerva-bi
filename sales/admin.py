...
class SalesOrderAdmin(AuditableMixin, admin.ModelAdmin):
    ...
    def _apply_currency(self, request, queryset, code: str):
        """Оновлює валюту замовлення і всіх рядків.
        SalesOrder.shipping_currency (вартість доставки) НЕ змінюється."""
        from .models import SalesOrderLine
    ...
    def documents_list(self, obj):
        ...
        js = (
            ...
            'window._refreshDocsPanel=function(pk){'
            ...
            'fc.id="sv-doc-pdf-frame";'
            'dt.appendChild(hd);dt.appendChild(fc);document.body.appendChild(dt);'
            'var ht=null,ce=null;'
            ...
            'function dshow(el,e){ce=el;var url=el.dataset.docPdf||"";document.getElementById("sv-doc-pdf-name").textContent=el.dataset.docName||"";document.getElementById("sv-doc-pdf-frame").innerHTML=url?"<iframe src=\'"+url+"\' style=\'display:block;width:320px;height:420px;border:none;background:#fff\' title=\'PDF\'></iframe>":"";dt.style.display="block";dpos(e);}'
            'function dhide(){dt.style.display="none";document.getElementById("sv-doc-pdf-frame").innerHTML="";ce=null;}'
    ...
    def doc_buttons(self, obj):
        ...
        js = (
            ...
            '}}'
            'if(d.local&&d.local.skip&&d.local.hint){'
            ...
            'font-size:11px;font-family:monospace;line-height:1.6;border-left:3px solid #37474f";'
            'var wrap=document.getElementById("savebtn-"+id);'
            'if(wrap)wrap.parentNode.insertAdjacentElement("afterend",dbg);}'
            'dbg.style.borderColor="#37474f";dbg.style.color="#607d8b";'
            'dbg.textContent="⏭️ "+d.local.skip;}'
            'if(d.url&&d.filename&&window.MinervaLocalSave&&MinervaLocalSave.supported){'
            'var _lsId="ls-dbg-"+id;'
            'var _lsEl=document.getElementById(_lsId);'
            'if(!_lsEl){_lsEl=document.createElement("div");_lsEl.id=_lsId;'
            '_lsEl.style.cssText="margin-top:4px;padding:5px 10px;border-radius:4px;'
            'font-size:11px;font-family:monospace;border-left:3px solid #1565c0";'
            ...
            '_lsEl.style.borderColor="#37474f";_lsEl.style.color="#607d8b";'
            '}else{'
            '_lsEl.textContent="⚠️ Локально: "+r.reason;'
            '_lsEl.style.borderColor="#f57f17";_lsEl.style.color="#ffb74d";}'
            '});}'
            'setTimeout(()=>{btn.disabled=false;btn.textContent="💾";},3000);'
            '}else{'
    ...
    def doc_templates_panel(self, obj):
        ...
        html = (
            ...
            f'<button type="button" onclick="DocumentWidget.loadTemplates(\'doc-templates-list\')" '
            f'title="Оновити список шаблонів" '
            f'style="font-size:11px;background:none;border:none;color:var(--text-dim);cursor:pointer;padding:2px 4px">🔄</button>'
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        try:
            ...
            if obj and obj.source == "digikey" and obj.status in ("processing", "shipped"):
                ...
                dk_confirmed = (obj.status == "shipped" and obj.status_source == "DigiKey Marketplace")
                extra_context["digikey_already_shipped"] = dk_confirmed
                extra_context["digikey_not_confirmed"] = (
                    not dk_confirmed and bool(obj.tracking_number)
                )
            if obj and obj.source == "digikey":
    ...
    def card_expand_view(self, request, pk):
        """AJAX GET — документи замовлення для розгорнутої секції картки."""
        order = get_object_or_404(SalesOrder, pk=pk)
        doc_list_url = reverse('admin:sales_salesorder_doc_list', args=[pk])

        docs_html = self._docs_panel_html(order)
        sep = '<div style="border-top:1px solid var(--border-strong,#2a3f52);margin:12px 0"></div>'
    ...
    def import_excel_view(self, request):
        ...
        ORDER_FIELDS = [
            ...
            ("client",            "Клієнт / Компанія (білінг)"),
            ("contact_name",      "Контактна особа (білінг)"),
            ("phone",             "Телефон (білінг)"),
            ("email",             "Email (білінг)"),
            ("ship_name",         "Ім'я отримувача посилки"),
            ("ship_company",      "Компанія отримувача"),
            ("ship_phone",        "Телефон доставки"),
            ("ship_email",        "Email доставки"),
